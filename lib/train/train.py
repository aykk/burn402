...
def run_retrieval(passages):
    ...
    model = {
        ...
        "metrics": [
            metric(
                "recallAt1",
                "of test questions, it put the right passage first",
                round(recall1, 4),
                "percent",
                better="higher",
                headline=True,
                detail="test questions are the opening sentence of a held-out passage, so real questions are harder",
            ),
            metric("recallAt3", "of test questions, the right passage was in its top three", round(recall3, 4), "percent", better="higher"),
            metric("crossValidated", "same test, averaged across folds while choosing settings", round(scores[best["id"]], 4), "percent", better="higher"),
            metric("passages", "passages it can answer from", len(passages), "count"),
            metric("pages", "pages they came from", len({p["url"] for p in passages}), "count"),
            metric("terms", "words and word pairs kept", len(keep), "count"),
            metric("probes", "held-out questions it was tested with", len(probes), "count"),
            metric("folds", "cross-validation folds per setting", folds, "count"),
            metric("configs", "settings compared", len(grid), "count"),
        ],
        "settings": [
            metric("maxTerms", "vocabulary size", best["maxTerms"], "count"),
            metric("minCount", "minimum times a word must appear", best["minCount"], "count"),
            metric("weightsPerPassage", "weights kept per passage", keep_n, "count"),
        ],
        "candidates": {
            "columns": [
...
def run_classifier(rows):
    ...
    model = {
        ...
        "metrics": [
            metric("heldOutAccuracy", "correct on rows it never saw", round(held, 4), "percent", better="higher", headline=True),
            metric("crossValidated", "cross-validated score while choosing settings", round(scores[best["id"]], 4), "percent", better="higher"),
            metric("rows", "rows it learned from", len(rows) - len(holdout_rows), "count"),
            metric("holdOutRows", "rows held back to test it", len(holdout_rows), "count"),
            metric("labels", "labels it can answer with", len(labels), "count"),
            metric("terms", "words and word pairs kept", len(keep), "count"),
            metric("folds", "cross-validation folds per setting", folds, "count"),
            metric("configs", "settings compared", len(grid), "count"),
        ],
...
def run_generator(text):
    ...
    model = {
        ...
        "metrics": [
            metric("heldOutPerplexity", "perplexity on text it never saw", round(held, 3), "number", better="lower", headline=True),
            metric("crossValidated", "cross-validated perplexity while choosing settings", round(-scores[best["id"]], 3), "number", better="lower"),
            metric("characters", "characters it learned from", len(train_text), "count"),
            metric("holdOutCharacters", "characters held back to test it", len(holdout_text), "count"),
            metric("contexts", "contexts kept in the table", len(table), "count"),
            metric("folds", "cross-validation folds per setting", folds, "count"),
            metric("configs", "settings compared", len(grid), "count"),
        ],
        "settings": [
            metric("order", "characters of context", best["order"], "count"),
